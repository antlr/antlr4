...
public class TestInterpreterDataReader extends BaseJavaTest {
	@Test
	public void testParseFile() throws NoSuchFieldException, IllegalAccessException, org.antlr.runtime.RecognitionException, IOException {
		String tempDirectory = Files.createTempDirectory("").toString();
		Grammar g = new Grammar(Paths.get(tempDirectory, "Calc.g4").toString(),
				"grammar Calc;\n" +
						"s :  expr EOF\n" +
						"  ;\n" +
						"expr\n" +
						...
						"WS : [ \\t]+ -> channel(HIDDEN);");
		g.name = "Calc";
		new Tool().generateInterpreterData(g);

		String interpFile = Paths.get(tempDirectory, g.name + ".interp").toString();
		InterpreterDataReader.InterpreterData interpreterData = InterpreterDataReader.parseFile(interpFile);
