...
public class TestFullContextParsing extends BaseTest {
	...
	String testFullContextIF_THEN_ELSEParse(String input) throws Exception {
		String grammar = "grammar T;\n" +
	                  "s \n" +
	                  "@init {Interpreter.PredictionMode = PredictionMode.LlExactAmbigDetection;}\n" +
	                  "@after {this.DumpDFA();}\n" +
	                  "	: '{' stat* '}' ;\n" +
	...
	public void testLoopsSimulateTailRecursion() throws Exception {
		String grammar = "grammar T;\n" +
	                  ...
	                  "	: expr_or_assign*;\n" +
	                  "expr_or_assign\n" +
	                  "	: expr '++' {Console.WriteLine(\"fail.\");}\n" +
	                  "	|  expr {Console.WriteLine(\"pass: \"+$expr.text);}\n" +
	                  "	;\n" +
	...
	public void testAmbiguityNoLoop() throws Exception {
		String grammar = "grammar T;\n" +
	                  "prog\n" +
	                  "@init {Interpreter.PredictionMode = PredictionMode.LlExactAmbigDetection;}\n" +
	                  "	: expr expr {Console.WriteLine(\"alt 1\");}\n" +
	                  "	| expr\n" +
	                  "	;\n" +
	                  "expr: '@'\n" +
	                  "	| ID '@'\n" +
	...
	String testExprAmbiguity(String input) throws Exception {
		String grammar = "grammar T;\n" +
	                  "s\n" +
	                  "@init {Interpreter.PredictionMode = PredictionMode.LlExactAmbigDetection;}\n" +
	                  ":   expr[0] {Console.WriteLine($expr.ctx.ToStringTree(this));};\n" +
	                  "	expr[int _p]\n" +
	                  "		: ID \n" +
	                  "		( \n" +
	                  "			{5 >= $_p}? '*' expr[6]\n" +
