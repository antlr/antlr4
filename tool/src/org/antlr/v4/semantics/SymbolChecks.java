...
public class SymbolChecks {
	 ...
	 * 2. Compare every string literal with each other ({@link #checkForOverlap(Grammar, Rule, Rule, List, List)})
	...
	private void checkPredicateLocation(Collection<Rule> rules) {
		HashMap<RuleAST, Boolean> calculatedRules = new HashMap<>();
		for (Rule rule : rules) {
			if (!rule.isFragment()) {
				checkPredicateLocation(rule.ast, calculatedRules, false);
			}
		}
	}

	/**
	 * Returns true if at least one warning was throw for any child
	 * */
	private boolean checkPredicateLocation(GrammarAST node, HashMap<RuleAST, Boolean> calculatedRules, boolean isFragment) {
		if (node instanceof RuleAST) {
			RuleAST ruleAST = (RuleAST) node;
			Boolean calculatedValue = calculatedRules.get(ruleAST);
			if (calculatedValue != null) {
			...
			calculatedRules.put(ruleAST, false); // Prevent endless recursion
			boolean result = checkPredicateLocation((GrammarAST) ruleAST.getChild(ruleAST.getChildCount() - 1), calculatedRules,
					isFragment);
			calculatedRules.put(ruleAST, result);
			return result;
		}
			...
			for (Object child : node.getChildren()) {
				result |= checkPredicateLocation((GrammarAST)child, calculatedRules, isFragment);
			}
			return result;
		}
		else if (node instanceof AltAST) {
			...
			for (Object o : node.getChildren()) {
				GrammarAST child = (GrammarAST) o;
				Rule refRule = null;
				boolean onlyPredicates = false;
				if (child instanceof TerminalAST) {
					if (child.token.getType() == ANTLRLexer.TOKEN_REF) {
						refRule = nameToRuleMap.get(child.token.getText());
						if (refRule != null) {
							onlyPredicates = checkPredicateLocation(refRule.ast, calculatedRules, refRule.isFragment());
						}
					}
				}
				if (refRule == null) {
					onlyPredicates = checkPredicateLocation(child, calculatedRules, isFragment);
				}
				// All predicates on first place should be collected thus no break here
				if (onlyPredicates) {
					result = true;
					if (refRule != null) { // Do not throw false positive warnings after any rule ref
						errMgr.grammarError(ErrorType.PREDICATE_AT_THE_BEGINNING_OF_LEXER_RULE_DEGRADES_PERFORMANCE, g.fileName,
								child.token, "of " + refRule.name);
						break;
					}
				}
				else {
					if (!(child instanceof ActionAST)) { // Ignore actions since they are executed after all predicates
						break;
					}
				}
			}
		...
		else if (node instanceof PlusBlockAST || node instanceof StarBlockAST || node instanceof OptionalBlockAST) {
			return checkPredicateLocation((GrammarAST) node.getChild(0), calculatedRules, isFragment);
		}
		else if (node instanceof PredAST) {
			if (!isFragment) {
				PredAST predAST = (PredAST) node;
				errMgr.grammarError(ErrorType.PREDICATE_AT_THE_BEGINNING_OF_LEXER_RULE_DEGRADES_PERFORMANCE, g.fileName, predAST.token, predAST.token.getText());
			}
			return true;
		}
		return false;
