...
size_t ParserATNSimulator::adaptivePredict(TokenStream *input, size_t decision, ParserRuleContext *outerContext) {
  ...
  if (s0 == nullptr) {
    ...
    std::unique_ptr<ATNConfigSet> s0_closure =
        computeStartState(dynamic_cast<ATNState *>(dfa.atnStartState), &ParserRuleContext::EMPTY, fullCtx);
    ...
    if (dfa.isPrecedenceDfa()) {
      ...
      dfa::DFAState *newState = new dfa::DFAState(
          applyPrecedenceFilter(ds0->configs.get())); /* mem-check: managed by the DFA or deleted below */
      s0 = addDFAState(dfa, newState);
      dfa.setPrecedenceStartState(parser->getPrecedence(), s0, _edgeLock);
      if (s0 != newState) {
        delete newState; // If there was already a state with this config set we don't need the new
                         // one.
      }
    } else {
      dfa::DFAState *newState =
          new dfa::DFAState(std::move(s0_closure)); /* mem-check: managed by the DFA or deleted below */
      s0 = addDFAState(dfa, newState);
...
#if DEBUG_ATN == 1 || DEBUG_LIST_ATN_DECISIONS == 1
  std::cout << "execATN decision " << dfa.decision << " exec LA(1)==" << getLookaheadName(input) << " line "
            << input->LT(1)->getLine() << ":" << input->LT(1)->getCharPositionInLine() << std::endl;
#endif
...
dfa::DFAState *ParserATNSimulator::computeTargetState(dfa::DFA &dfa, dfa::DFAState *previousD, size_t t) {
  ...
  dfa::DFAState *D = new dfa::DFAState(
      std::move(reach)); /* mem-check: managed by the DFA or deleted below, "reach" is no longer valid now. */
...
void ParserATNSimulator::predicateDFAState(dfa::DFAState *dfaState, DecisionState *decisionState) {
  ...
  std::vector<Ref<SemanticContext>> altToPred =
      getPredsForAmbigAlts(altsToCollectPredsFrom, dfaState->configs.get(), nalts);
  if (!altToPred.empty()) {
...
size_t ParserATNSimulator::execATNWithFullContext(dfa::DFA &dfa, dfa::DFAState *D, ATNConfigSet *s0, TokenStream *input,
...
ATNConfigSet *ParserATNSimulator::removeAllConfigsNotInRuleStopState(ATNConfigSet *configs, bool lookToEndOfRule) {
...
ParserATNSimulator::getPredicatePredictions(const antlrcpp::BitSet &ambigAlts,
  ...
  for (size_t i = 1; i < altToPred.size(); ++i) {
    ...
    if (ambigAlts.test(i)) {
      pairs.push_back(new dfa::DFAState::PredPrediction(
          pred, (int)i)); /* mem-check: managed by the DFAState it will be assigned to after return */
    }
  }
  return pairs;
}
...
void ParserATNSimulator::closureCheckingStopState(Ref<ATNConfig> const &config, ATNConfigSet *configs,
                                                  ATNConfig::Set &closureBusy, bool collectPredicates, bool fullCtx,
  ...
  if (is<RuleStopState *>(config->state)) {
    ...
    if (!config->context->isEmpty()) {
      for (size_t i = 0; i < config->context->size(); i++) {
        ...
        Ref<ATNConfig> c =
            std::make_shared<ATNConfig>(returnState, config->alt, newContext.lock(), config->semanticContext);
    ...
    if (c != nullptr) {
      ...
      if (is<RuleStopState *>(config->state)) {
        ...
        configs->dipsIntoOuterContext =
            true; // TODO: can remove? only care when we add to set per middle of this method
...
bool ParserATNSimulator::canDropLoopEntryEdgeInLeftRecursiveRule(ATNConfig *config) const {
  ...
  for (size_t i = 0; i < numCtxs; i++) { // for each stack context
    ...
    if (returnStateTarget->getStateType() == ATNState::BLOCK_END && returnStateTarget->transitions.size() == 1 &&
...
Ref<ATNConfig> ParserATNSimulator::getEpsilonTarget(Ref<ATNConfig> const &config, Transition *t, bool collectPredicates,
  ...
  case Transition::PRECEDENCE:
    return precedenceTransition(config, static_cast<PrecedencePredicateTransition *>(t), collectPredicates, inContext,
                                fullCtx);
...
#if DEBUG_DFA == 1
  std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->precedence << ">=_p"
            << ", ctx dependent=true" << std::endl;
  if (parser != nullptr) {
    std::cout << "context surrounding pred is " << Arrays::listToString(parser->getRuleInvocationStack(), ", ")
              << std::endl;
  }
#endif
...
Ref<ATNConfig> ParserATNSimulator::predTransition(Ref<ATNConfig> const &config, PredicateTransition *pt,
                                                  bool collectPredicates, bool inContext, bool fullCtx) {
#if DEBUG_DFA == 1
  std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->ruleIndex << ":" << pt->predIndex
...
std::string ParserATNSimulator::getLookaheadName(TokenStream *input) { return getTokenName(input->LA(1)); }
...
void ParserATNSimulator::reportAttemptingFullContext(dfa::DFA &dfa, const antlrcpp::BitSet &conflictingAlts,
  ...
  std::cout << "reportAttemptingFullContext decision=" << dfa.decision << ":" << configs
            << ", input=" << parser->getTokenStream()->getText(interval) << std::endl;
#endif

  if (parser != nullptr) {
    parser->getErrorListenerDispatch().reportAttemptingFullContext(parser, dfa, startIndex, stopIndex, conflictingAlts,
                                                                   configs);
  }
}
...
void ParserATNSimulator::reportContextSensitivity(dfa::DFA &dfa, size_t prediction, ATNConfigSet *configs,
  ...
  std::cout << "reportContextSensitivity decision=" << dfa.decision << ":" << configs
            << ", input=" << parser->getTokenStream()->getText(interval) << std::endl;
#endif

  if (parser != nullptr) {
    parser->getErrorListenerDispatch().reportContextSensitivity(parser, dfa, startIndex, stopIndex, prediction,
                                                                configs);
  }
}
...
void ParserATNSimulator::setPredictionMode(PredictionMode newMode) { _mode = newMode; }
...
#ifdef _MSC_VER
#pragma warning(disable : 4996) // 'getenv': This function or variable may be unsafe. Consider using
                                // _dupenv_s instead.
#endif
