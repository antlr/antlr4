...
Token *UnbufferedTokenStream::get(size_t i) const { // get absolute index
  ...
  if (i < bufferStartIndex || i >= bufferStartIndex + _tokens.size()) {
    throw IndexOutOfBoundsException(std::string("get(") + std::to_string(i) + std::string(") outside buffer: ") +
                                    std::to_string(bufferStartIndex) + std::string("..") +
...
TokenSource *UnbufferedTokenStream::getTokenSource() const { return _tokenSource; }

std::string UnbufferedTokenStream::getText() { return ""; }

std::string UnbufferedTokenStream::getText(RuleContext *ctx) { return getText(ctx->getSourceInterval()); }
...
///  {@code p} index is {@code tokens.length-1}.  {@code p+need-1} is the tokens index 'need'
///  elements ahead.  If we need 1 element, {@code (p+1-1)==p} must be less than {@code
///  tokens.length}.
/// </summary>
void UnbufferedTokenStream::sync(ssize_t want) {
  ssize_t need = (static_cast<ssize_t>(_p) + want - 1) - static_cast<ssize_t>(_tokens.size()) +
...
size_t UnbufferedTokenStream::size() { throw UnsupportedOperationException("Unbuffered stream cannot know its size"); }

std::string UnbufferedTokenStream::getSourceName() const { return _tokenSource->getSourceName(); }
...
size_t UnbufferedTokenStream::getBufferStartIndex() const { return _currentTokenIndex - _p; }
