...
// which is itself based off of
// https://go.googlesource.com/go/+/refs/heads/master/src/unicode/utf8/utf8.go. If for some reason
// you feel the need to copy this implementation, please retain a comment referencing the two source
// files and giving credit, as well as maintaining any and all obligations required by the BSD
// 3-clause license that governs this file.
...
#undef ACCEPT
    constexpr std::pair<uint8_t, uint8_t> ACCEPT[16] = {
        {LOW, HIGH}, {0xa0, HIGH}, {LOW, 0x9f}, {0x90, HIGH}, {LOW, 0x8f}, {0x0, 0x0}, {0x0, 0x0}, {0x0, 0x0},
  ...
  std::pair<char32_t, size_t> Utf8::decode(std::string_view input) {
    ...
    if (size <= 1) {
      return {(static_cast<char32_t>(b & MASK2) << 6) | static_cast<char32_t>(b1 & MASKX), 2};
    }
    ...
    if (size <= 2) {
      return {(static_cast<char32_t>(b & MASK3) << 12) | (static_cast<char32_t>(b1 & MASKX) << 6) |
    ...
    return {(static_cast<char32_t>(b & MASK4) << 18) | (static_cast<char32_t>(b1 & MASKX) << 12) |
                (static_cast<char32_t>(b2 & MASKX) << 6) | static_cast<char32_t>(b3 & MASKX),
            4};
  }
  ...
  std::string &Utf8::encode(std::string *buffer, char32_t codePoint) {
    ...
    if (codePoint <= 0x7f) {
      ...
      buffer->push_back(static_cast<char>(T2 | static_cast<uint8_t>(codePoint >> 6)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint) & MASKX)));
    } else if (codePoint <= 0xffff) {
      buffer->push_back(static_cast<char>(T3 | static_cast<uint8_t>(codePoint >> 12)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint >> 6) & MASKX)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint) & MASKX)));
    } else {
      buffer->push_back(static_cast<char>(T4 | static_cast<uint8_t>(codePoint >> 18)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint >> 12) & MASKX)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint >> 6) & MASKX)));
      buffer->push_back(static_cast<char>(TX | (static_cast<uint8_t>(codePoint) & MASKX)));
    }
    return *buffer;
