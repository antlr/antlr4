...
class ParserATNSimulator extends ATNSimulator {
  ...
  ParserATNSimulator(Parser this.parser, ATN atn, this.decisionToDFA,
  ...
  int adaptivePredict(
    ...
    if (debug || debug_list_atn_decisions) {
      log("adaptivePredict decision $decision" +
          " exec LA(1)==" +
    ...
    try {
      ...
      if (s0 == null) {
        ...
        if (debug || debug_list_atn_decisions) {
          log("predictATN decision ${dfa.decision}" +
              " exec LA(1)==" +
              getLookaheadName(input_) +
              ", outerContext=" +
        ...
        bool fullCtx = false;
        ATNConfigSet s0_closure = computeStartState(
        ...
        if (dfa.isPrecedenceDfa()) {
          ...
          s0 = addDFAState(dfa, new DFAState(configs: s0_closure));
      ...
      int alt = execATN(dfa, s0, input_, index, outerContext);
      if (debug)
  ...
  int execATN(DFA dfa, DFAState s0, TokenStream input, int startIndex,
    ...
    if (debug || debug_list_atn_decisions) {
      log("execATN decision ${dfa.decision}" +
          " exec LA(1)==" +
    ...
    while (true) {
      ...
      if (D == ATNSimulator.ERROR) {
        ...
        int alt = getSynValidOrSemInvalidAltThatFinishedDecisionEntryRule(
      ...
      if (D.requiresFullContext && predictionMode != PredictionMode.SLL) {
        ...
        int alt = execATNWithFullContext(
  ...
  DFAState computeTargetState(DFA dfa, DFAState previousD, int t) {
    ...
    DFAState D = new DFAState(configs: reach);

    int predictedAlt = getUniqueAlt(reach);

    if (debug) {
      List<BitSet> altSubSets =
          PredictionModeExtension.getConflictingAltSubsets(reach);
      log("SLL altSubSets=$altSubSets" +
          ", configs=$reach" +
          ", predict=$predictedAlt, allSubsetsConflict=${PredictionModeExtension.allSubsetsConflict(altSubSets)}" +
...
//			log("reach="+reach+", "+reach.conflictingAlts, level: Level.SEVERE.value);
  ...
  ATNConfigSet computeReachSet(ATNConfigSet config, int t, bool fullCtx) {
    ...
    for (ATNConfig c in config) {
      ...
      for (int ti = 0; ti < n; ti++) {
        ...
        if (target != null) {
          intermediate.add(new ATNConfig.dup(c, state: target), mergeCache);
        }
      }
    }
    ...
    for (ATNConfig config in configs) {
      ...
      if (lookToEndOfRule && config.state.onlyHasEpsilonTransitions()) {
        ...
        if (nextTokens.contains(Token.EPSILON)) {
          ...
          result.add(
              new ATNConfig.dup(config, state: endOfRuleState), mergeCache);
        }
      }
    }
  ...
  ATNConfigSet applyPrecedenceFilter(ATNConfigSet configs) {
    Map<int, PredictionContext> statesFromAlt1 = {};
    ...
    for (ATNConfig config in configs) {
      ...
      if (updatedContext != config.semanticContext) {
        configSet.add(
            new ATNConfig.dup(config, semanticContext: updatedContext),
            mergeCache);
      } else {
  ...
  List<SemanticContext> getPredsForAmbigAlts(
    ...
    /* altToPred starts as an array of all null contexts. The entry at index i
		 ...
		 */
    List<SemanticContext> altToPred = List(nalts + 1);
  ...
  Pair<ATNConfigSet, ATNConfigSet> splitAccordingToSemanticValidity(
    ...
    for (ATNConfig c in configs) {
      if (c.semanticContext != SemanticContext.NONE) {
        bool predicateEvaluationResult = evalSemanticContextOne(
...
//          assert(newDepth > int.MIN_VALUE);
  ...
  bool canDropLoopEntryEdgeInLeftRecursiveRule(ATNConfig config) {
    ...
    // that p is in.
    int numCtxs = config.context.length;
  ...
  ATNConfig getEpsilonTarget(
    ...
    switch (t.type) {
      ...
      case TransitionType.EPSILON:
        return new ATNConfig.dup(config, state: t.target);
  ...
  ATNConfig ruleTransition(ATNConfig config, RuleTransition t) {
    ...
    return new ATNConfig.dup(config, state: t.target, context: newContext);
  }
  ...
  String getTokenName(int t) {
    ...
    Vocabulary vocabulary = parser != null
        ? parser.vocabulary
      ...
      ATNConfigSet configs) // configs that LL not SLL considered conflicting
  {
    if (debug || retry_debug) {
      Interval interval = Interval.of(startIndex, stopIndex);
      log("reportAmbiguity $ambigAlts:$configs" +
          ", input=" +
          parser.tokenStream.getText(interval));
    }
    if (parser != null)
...
// dup configs, tossing out semantic predicates
        ...
        for (ATNConfig c in configs) {
          c = new ATNConfig.dup(c, semanticContext: SemanticContext.NONE);
          dup.add(c);
        }
        configs = dup;
      }
...
// pure SLL or combined SLL+LL mode parsing
    ...
    List<BitSet> altsets = getConflictingAltSubsets(configs);
    bool heuristic =
