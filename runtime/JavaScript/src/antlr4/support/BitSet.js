...
/**
 * Throws if the given value is not a valid bit index.
 *
 * @param {number} values - The bit index.
 */
function assertBitIndex(value) {
  if (!Number.isInteger(value) || value < 0) {
    throw new TypeError('expected bit index to be zero or positive integer: ' + typeof value + ': ' + String(value));
  }
}

function assertBitValue(value) {
  if (!(typeof value == 'boolean') && !(value instanceof Boolean)) {
    throw new TypeError('expected bit value to be boolean: ' + typeof value + ': ' + String(value));
  }
}

function assertBitSet(value) {
  if (!(value instanceof BitSet)) {
    throw new TypeError('expected to be an instance of BitSet: ' + typeof value + ': ' + String(value));
  }
}
...
class BitSet {
  ...
  /**
   * Returns a hash value of the bitset.
   *
   * @returns {number}
   */
  hashCode() {
