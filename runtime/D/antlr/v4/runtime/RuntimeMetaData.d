...
class RuntimeMetaData
{
    ...
    public static void checkVersion(string generatingToolVersion, string compileTimeVersion)
    {
        ...
        if (runtimeConflictsWithGeneratingTool) {
            stderr.writefln("ANTLR Tool version %s used for code generation does not match the current runtime version %s",
                              generatingToolVersion, runtimeVersion);
        }
        if ( runtimeConflictsWithCompileTimeTool ) {
            stderr.writefln("ANTLR Runtime version %s used for parser compilation does not match the current runtime version %s",
     ...
     */
    public static string getMajorMinorVersion(string antlr_version)
    {
        size_t firstDot = antlr_version.indexOf('.');
        size_t secondDot = firstDot >= 0 ? antlr_version.indexOf('.', firstDot + 1) : -1;
        size_t firstDash = antlr_version.indexOf('-');
        size_t referenceLength = antlr_version.length;
        if (secondDot >= 0) {
