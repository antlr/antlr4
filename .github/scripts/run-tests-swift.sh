...
if [ $rc == 0 ]; then
  ...
  if [ $GROUP == "LEXER" ]; then
      mvn -e -X -q -Dgroups="org.antlr.v4.test.runtime.category.LexerTests" -Dtest="swift.*" test
  elif [ $GROUP == "PARSER" ]; then
      mvn -e -X -q -Dgroups="org.antlr.v4.test.runtime.category.ParserTests" -Dtest="swift.*" test
  elif [ $GROUP == "RECURSION" ]; then
      mvn -e -X -q -Dgroups="org.antlr.v4.test.runtime.category.LeftRecursionTests" -Dtest="swift.*" test
  else
      mvn -e -X -q -Dtest=swift.* test
  fi
  rc=$?
