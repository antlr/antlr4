...
if [ $rc == 0 ]; then
  ...
  if [ $GROUP == "LEXER" ]; then
      ...
      mvn -e -q -Dgroups="org.antlr.v4.test.runtime.category.ParserTestsBatch1" -Dtest="swift.**" test
  elif [ $GROUP == "PARSER2" ]; then
      mvn -e -q -Dgroups="org.antlr.v4.test.runtime.category.ParserTestsBatch2" -Dtest="swift.**" test
